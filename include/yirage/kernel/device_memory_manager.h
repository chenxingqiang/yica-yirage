...
private:
  ...
  // communication
  char *data_base_ptr[yirage::config::MAX_NUM_GPUS];
  char *fp_base_ptr[yirage::config::MAX_NUM_GPUS];
  char *stensor_fp_base_ptr;

public:
  cudaStream_t stream[yirage::config::MAX_NUM_GPUS];
  // cublasHandle_t blas[yirage::config::MAX_NUM_GPUS];
  // cudnnHandle_t cudnn;
};
