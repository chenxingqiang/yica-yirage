...
def get_compile_command(
    ...
    common_cmd = [
        ...
        f"-I{py_include_dir}",
        f"-I{yirage_inc_path}",
        f"-I{os.path.join(yirage_inc_path, 'yirage/persistent_kernel')}",
        f"-I{os.path.join(yirage_deps_path, 'cutlass/include')}",
        f"-I{os.path.join(yirage_home_path, 'deps/json/include')}",
...
class PersistentKernel:
    def __init__(
        ...
        backend: str = None  # New parameter for backend selection
    ):
    ...
    def _compile_cuda(self, **kwargs):
        ...
        # find yirage home
        if "YIRAGE_HOME" in os.environ:
            YIRAGE_HOME_PATH = os.environ.get("YIRAGE_HOME")
        else:
            raise RuntimeError(
                "YIRAGE_HOME unspecified; Please set YIRAGE_HOME to be the root of the YiRage folder"
            )
    ...
    def __call__(self, **kwargs):
        ...
        if self.profiler_tensor is not None:
            ...
            export_to_perfetto_trace(
                self.profiler_tensor, f"yirage_{self.mpi_rank}.perfetto-trace"
            )
