...
def get_cc_cmd(
    ...
    common_cmd = [
        ...
        "-O3",
        f"-I{py_include_dir}",
        f"-I{os.path.join(INCLUDE_PATH, 'yirage/transpiler/runtime')}",
...
class KNGraph:
    ...
    def cuda_call(self, **kwargs):
        ...
        if results["profiler_buf_size"] > 0:
            ...
            profiler_result_file = os.path.join(
                profiler_result_dir, "yirage.perfetto-trace"
            )
    ...
    def superoptimize(
        ...
        if use_cached_graphs:
            previous_checkpoint = "yirage_cached_mugraphs_{:x}.json".format(
        ...
        if backend == "cuda":
            ...
            os.environ["KERNELS_PATH"] = os.path.join(
                INCLUDE_PATH, "yirage/triton_transpiler/runtime"
            )  # for triton
            ...
            spec = importlib.util.spec_from_file_location(
                "__yirage_launcher", best_file_path
            )
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
