...
Existing ML systems generally launch RMSNorm and the following MatMul as two separate kernels, since RMSNorm includes reduction/broadcast, making it hard to directly fuse it with other computation. We can use YiRage to automatically generate a highly optimized kernel that fuses RMSNorm and MatMul:
...
.. code-block:: Python
    ...
    W = graph.new_input(dims=(406, 4096), dtype=yr.float16)
    O = graph.matmul(graph.rms_norm(X, G), W)
    graph.mark_output(O)
    optimized_graph = yr.superoptimize(graph)

YiRage introduces uGraph, a multi-level graph structure to represent the computation of a GPU kernel. The uGraph of the discovered kernel is as follows. Instead of launching separate kernels, the YiRage-discovered kernel leverages the commutativity of the division in RMSNorm and the multiplication in MatMul, moving division after MatMul. This allows YiRage to load the input tensor X once to compute both the denominator of RMSNorm and MatMul. The optimized kernel is 1.5-1.7x faster than the original kernel.
...
The object `optimized_graph` can directly run as a function, and doing so will let YiRage transpile the uGraph into CUDA code, compile the code for execution, and launch the compiled kernel.
