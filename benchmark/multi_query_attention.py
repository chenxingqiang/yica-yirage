...
if __name__ == "__main__":
    ...
    Q = graph.new_input(dims=(batch_size, 1024, 64), dtype=yr.float16)
    K = graph.new_input(dims=(batch_size, 64, 4096), dtype=yr.float16)
    V = graph.new_input(dims=(batch_size, 4096, 64), dtype=yr.float16)
    A = graph.matmul(Q, K)
    E = graph.exp(A)
