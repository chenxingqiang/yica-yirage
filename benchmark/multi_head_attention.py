...
if __name__ == "__main__":
    ...
    parser.add_argument('--file', type=str, default='multi_head_attention.json')
    ...
    Q = graph.new_input(dims=(32 * batch_size, 16, 64), dtype=yr.float16)
    K = graph.new_input(dims=(32 * batch_size, 64, 4096), dtype=yr.float16)
    V = graph.new_input(dims=(32 * batch_size, 4096, 64), dtype=yr.float16)
    A = graph.matmul(Q, K)
    E = graph.exp(A)
    ...
    input_tensors = [
        torch.randn(32 * batch_size, 16, 64, dtype=torch.float16, device='cuda:0'),
        torch.randn(32 * batch_size, 64, 4096, dtype=torch.float16, device='cuda:0'),
        torch.randn(32 * batch_size, 4096, 64, dtype=torch.float16, device='cuda:0')
    ]
