...
def build_yirage_graph(model, world_size, rank, args, tokens_tensor, step_tensor):
    ...
    embed_out = mpk.new_tensor(dims=(batch_size, hidden_size), dtype=yr.bfloat16, name="embed_out")
    attn_in = mpk.new_tensor(dims=(batch_size, fused_outdim_1 // world_size), dtype=yr.bfloat16, name="attn_in")
    attn_out = mpk.new_tensor(dims=(batch_size, num_local_q_heads * head_dim), dtype=yr.bfloat16, name="attn_out")

    is_nvshmem = "nvshmem_tensor" if world_size > 1 else "cuda_tensor"
    attn_proj_out = mpk.new_tensor(dims=(batch_size, hidden_size), dtype=yr.bfloat16, name="attn_proj_out", io_category=is_nvshmem)
    allreduce_buf = mpk.new_tensor(dims=(world_size, batch_size, hidden_size), dtype=yr.bfloat16, name="all_reduce_buf", io_category=is_nvshmem)
    attn_allreduce_out = mpk.new_tensor(dims=(batch_size, hidden_size), dtype=yr.bfloat16, name="attn_allreduce_out", io_category=is_nvshmem)
    mlp_mid = mpk.new_tensor(dims=(batch_size, fused_outdim_2 // world_size), dtype=yr.bfloat16, name="mlp_mid")
    mlp_out = mpk.new_tensor(dims=(batch_size, hidden_size), dtype=yr.bfloat16, name="mlp_out", io_category=is_nvshmem)
    mlp_final = mpk.new_tensor(dims=(batch_size, hidden_size), dtype=yr.bfloat16, name="mlp_final", io_category=is_nvshmem)
    argmax_in = mpk.new_tensor(dims=(batch_size, vocab_size), dtype=yr.bfloat16, name="argmax_in")
    argmax_part_value = mpk.new_tensor(dims=(batch_size, 96), dtype=yr.bfloat16, name="argmax_part_value")
    argmax_part_index = mpk.new_tensor(dims=(batch_size, 96), dtype=yr.int64, name="argmax_part_index")
    argmax_out = mpk.new_tensor(dims=(batch_size, 1), dtype=yr.int64, name="argmax_out")
...
def run_yirage_generation(model, mpk, tokens, prompt_len, step_tensor, position_embeddings):
    starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
    stream = torch.cuda.Stream()
...
def main():
    parser = argparse.ArgumentParser(description="Interactive chat demo for Qwen3-8B with YiRage.")
    parser.add_argument("--use-yirage", action="store_true", help="Use YiRage kernels for inference.")
    ...
    mpk = None
    if args.use_yirage:
        mpk = build_yirage_graph(model, world_size, rank, args, tokens, step_tensor)
    ...
    if rank == 0:
        print("\n> Welcome to the YiRage Qwen3-8B chat demo. Type 'exit' or 'quit' to end.")
    ...
    while True:
        ...
        if args.use_yirage:
            end_pos, run_time, generated_len = run_yirage_generation(
